...
function reconcileControls(){if(["PLAN","TEACH","REVIEW","APPLY","TRANSFER_TEST"].includes(state.phase)){el("inputLabel").textContent="Próxima etapa";show(el("inputPanel"),true);show(el("send"),false);show(el("preview"),false);show(el("continue"),true)}else if(state.phase==="PRACTICE"){const hasExercise=!el("exercise").classList.contains("hidden")&&el("exercise").textContent.trim()!=="";show(el("inputPanel"),true);show(el("send"),hasExercise);show(el("preview"),hasExercise);show(el("continue"),!hasExercise)}}
...
function renderEvent(event){const data=event.data||{};if(event.type==="session_resumed"){setMarkdown(el("message"),data.message||"Sessão retomada.");show(el("inputPanel"),true);show(el("send"),false);show(el("preview"),false);show(el("continue"),true)}else if(event.type==="probe_question"){el("message").textContent="Pergunta "+text(data.number)+"/"+text(data.max_questions)+"\\n\\n"+text(data.question);el("inputLabel").textContent="Sua resposta";el("answer").id="answer";el("answer").value="";el("answer").placeholder="Explique com suas palavras";show(el("inputPanel"),true);show(el("send"),true);show(el("preview"),false);show(el("continue"),false)}else if(event.type==="probe_completed"){setMarkdown(el("message"),data.summary||data.message||"Diagnóstico concluído. Avance para o plano pedagógico.");el("inputLabel").textContent="Próxima etapa";el("answer").id="answer";el("answer").value="";show(el("inputPanel"),true);show(el("send"),false);show(el("preview"),false);show(el("continue"),true)}else if(["plan","teach","review","apply","transfer_test"].includes(event.type)){setMarkdown(el("message"),text(data.message)+(data.comprehension_check?"\\n\\nChecagem: "+data.comprehension_check:""));show(el("inputPanel"),true);show(el("send"),false);show(el("preview"),false);show(el("continue"),true)}else if(event.type==="exercise"){setMarkdown(el("exercise"),"Exercício\\n\\nObjetivo: "+text(data.objective)+"\\n\\n"+text(data.statement)+"\\n\\nDificuldade "+text(data.difficulty)+" · "+text((data.concepts||[]).join(", ")));show(el("exercise"),true);el("inputLabel").textContent="Sua SQL";el("answer").id="sql";el("sql").value="";el("sql").placeholder="SELECT ...";show(el("inputPanel"),true);show(el("send"),true);show(el("preview"),true);show(el("continue"),false)}else if(event.type==="preview_execution"||event.type==="execution"){el("message").textContent=event.type==="preview_execution"?"Prévia da execução":"Resultado da avaliação";el("message").innerHTML+="\\n\\n"+(data.error?"Erro: "+escapeHtml(data.error.message||data.error.category):renderRows(data))}else if(event.type==="feedback"){setMarkdown(el("message"),data.message)}else if(event.type==="progress"){el("progress").textContent="Progresso\\n\\n"+(data.concepts||[]).map(item=>text(item.concept)+": "+text(Number(item.mastery).toFixed(2))+" ("+text(item.confidence)+")").join("\\n");show(el("progress"),true)}else if(event.type==="decision"){el("message").innerHTML+="<p>Próxima ação: "+escapeHtml(data.action||data.next_action)+"</p>"}else if(event.type==="error"){el("error").textContent=text(data.message||"A operação não pôde ser concluída.");show(el("error"),true)}}
function renderResult(result){show(el("error"),false);if(result.session_id)state.id=result.session_id;if(result.session&&result.session.phase)state.phase=result.session.phase;if(result.phase)state.phase=result.phase;show(el("content"),true);setStatus();(result.events||[]).forEach(renderEvent);reconcileControls();el("phaseTitle").textContent=phaseNames[state.phase]||"Sessão"}
...
function createResumeEvent(application) { return { type: "session_resumed", data: { session_id: application.session?.id, phase: phaseOf(application), message: "Sessão recuperada do estado persistido." } }; }
  ...
  return createServer(async (request, response) => {
    try {
      ...
      if (request.method === "POST" && sessionId && match?.[2] === "resume") {
        if (sessions.has(sessionId)) return send(response, 200, { session_id: sessionId, phase: phaseOf(sessions.get(sessionId)), events: [createResumeEvent(sessions.get(sessionId))], session: sessions.get(sessionId).session });
        const application = await applicationFactory({ env });
        const result = await application.resume(sessionId);
        sessions.set(sessionId, application);
        return send(response, 200, { session_id: sessionId, phase: phaseOf(application), events: result.events, session: result.session });
      }
